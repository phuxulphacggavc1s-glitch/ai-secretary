...
function StatCard({ value, label, alert, onClick }) {
  ...
  return (
    <Component
      ...
      } ${onClick ? 'cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary/30' : ''}`}
    >
      <div
      ...
      <div className={`mt-1.5 text-xs ${alert ? 'font-semibold text-rose-500' : 'text-slate-500'}`}>
        {label}
      </div>
    </Component>
  )
...
export default function SecretaryBriefing({
  ...
  return (
    <section className="space-y-3.5">
      {/* Compact stats */}
      <div className="rounded-xl2 border border-slate-200/80 bg-white p-4 shadow-soft">
        <div className="mb-3 flex items-center gap-2 text-xs text-slate-400">
          <Sparkles size={14} className="text-primary" />
          今日概况 · {dayjs().format('M月D日 dddd')}
        </div>
